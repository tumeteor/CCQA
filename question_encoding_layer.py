...
class question_encoding(object):
    def __init__(self, config):
        logging.warning('begin initialize the question_encoding')
        ...
        logging.warning('the question bp_states:'.format(self.bp_states_h.get_shape()))
        self.td_states_h = self.td_lstm.states_h
        self.config = config
        self.nodes_states = self.get_tree_states(self.bp_states_h, self.td_states_h)
        logging.warning('the question_encoding initialization finished')
...
class top_down_lstm(object):
    ...
    def add_placeholders(self):
        self.t_input = tf.placeholder(tf.int32, [self.config.maxnodesize], name='td_input')
        self.t_treestr = tf.placeholder(tf.int32, [self.config.maxnodesize], name='td_tree')
        self.t_par_leaf = tf.placeholder(tf.int32, [self.config.maxnodesize], name='td_par_leaf')
    ...
    def compute_states(self, emb_leaves):
        ...
        logging.warning('process leaves done')
...
class bottom_up_lstm(object):
    ...
    def add_placeholders(self):
        self.input = tf.placeholder(tf.int32, [self.config.maxnodesize], name='input')
        self.treestr = tf.placeholder(tf.int32, [self.config.maxnodesize, 2], name='tree')
    ...
    def add_embedding(self):
        ...
        with tf.variable_scope("Embed", regularizer=None):
            ...
            embedding = tf.get_variable('embedding', initializer=self.config.embedding, trainable=False,
                                        regularizer=None)
