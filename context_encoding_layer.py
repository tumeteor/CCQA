...
class context_encoding(object):
    def __init__(self, config):
        ...
        with tf.variable_scope('context_bidirectional_chain_lstm'):
            ...
            chain_outputs, chain_state = tf.nn.bidirectional_dynamic_rnn(self.fwcell, self.bwcell, self.inputs,
                                                                         self.sentence_num_batch,
                                                                         initial_state_fw=self._fw_initial_state,
                                                                         initial_state_bw=self._bw_initial_state)
        ...
        self.sentences_final_states = self.get_tree_states(self.c_bp_lstm.sentences_hidden_states,
                                                           self.c_td_lstm.sentences_hidden_states)

    def get_tree_states(self, sentences_bp_states, sentences_td_states):
        states = tf.concat(values=[sentences_bp_states, sentences_td_states], axis=2)
        return states
...
class context_bottom_up_lstm(object):
    ...
    def add_placeholders(self):
        ...
        self.input = tf.placeholder(tf.int32, [None, self.config.maxnodesize], name='context_input')
        self.input = tf.gather(self.input, tf.range(self.sentence_num))
        self.treestr = tf.placeholder(tf.int32, [None, self.config.maxnodesize, 2], name='context_tree')
    ...
    def get_sentences_root_states(self, sentences_states):
        def _get_root_states(x):
            x = tf.gather(x, tf.range(tf.to_int32(
                tf.divide(tf.reduce_sum(tf.to_int32(tf.not_equal(x, -1.0))), tf.constant(self.hidden_dim)))))
    ...
    def compute_sentences_states(self, emb_batch):
        ...
        states_h = tf.reshape(
            tf.pad(states_h, [[0, 300 - tf.gather(tf.shape(states_h), 0)], [0, 0]], "CONSTANT", constant_values=-1),
            [-1, 150])
        states_h = tf.expand_dims(states_h, 0)
        ...
        def _computestates(states, emb_batch, idx_batch):
            ...
            cur_states = tf.reshape(
                tf.pad(cur_states, [[0, 300 - tf.gather(tf.shape(cur_states), 0)], [0, 0]], "CONSTANT",
                       constant_values=-1), [-1, 150])
            cur_states = tf.expand_dims(cur_states, 0)
    ...
    def compute_states(self, emb, idx_batch=0, fillnum=300):
        dim2 = fillnum
        # nodes_h_final = np.empty([dim2, 150], dtype='float32')
        # nodes_h_final.fill(-1)
...
class context_top_down_lstm(object):
    ...
    def add_placeholders(self):
        self.t_input = tf.placeholder(tf.int32, [None, self.config.maxnodesize], name='context_td_input')
        self.t_input = tf.gather(self.t_input, tf.range(self.sentence_num))
        self.t_treestr = tf.placeholder(tf.int32, [None, self.config.maxnodesize], name='context_td_tree')
        self.t_treestr = tf.gather(self.t_treestr, tf.range(self.sentence_num))
        self.t_par_leaf = tf.placeholder(tf.int32, [None, self.config.maxnodesize], name='context_td_par_leaf')
    ...
    def compute_sentences_states_h(self, emb_leaves):
        ...
        nodes_h = tf.reverse(nodes_h, axis=[0])
        nodes_h = tf.reshape(
            tf.pad(nodes_h, [[0, 300 - tf.gather(tf.shape(nodes_h), 0)], [0, 0]], "CONSTANT", constant_values=-1),
            [-1, 150])
        nodes_h_states = tf.expand_dims(nodes_h, axis=0)
        logging.warning('{}'.format(nodes_h_states.shape))
        logging.warning('expand_dims done')
        ...
        def _tdcomputestate(idx_curbatch, nodes_h_states):
            ...
            tmpnodes_h = tf.reverse(tmpnodes_h, axis=[0])
            tmpnodes_h = tf.reshape(
                tf.pad(tmpnodes_h, [[0, 300 - tf.gather(tf.shape(tmpnodes_h), 0)], [0, 0]], "CONSTANT",
                       constant_values=-1), [-1, 150])
    ...
    def process_leafs(self, inodes_h, inodes_c, emb_leaves, idx_batch):
        logging.warning('begin get num leaves')
        num_leaves = tf.squeeze(tf.gather(self.num_leaves, idx_batch))
        logging.warning('get num leaves done')
        embx = tf.gather(tf.gather(emb_leaves, idx_batch), tf.range(num_leaves))
        logging.warning('get leaf embedding done')
        leaf_parent = tf.gather(tf.gather(self.t_par_leaf, idx_batch), tf.range(num_leaves))
        logging.warning('get leaf parents array done')
        ...
        with tf.variable_scope('td_Composition', reuse=True):
            ...
            logging.warning('begin enumerate the idx_var')

            def _recurceleaf(node_h, node_c, idx_var):
                node_info = tf.gather(leaf_parent, idx_var)
                logging.warning('get t_idx, the index of parent')
                ...
                node_c = tf.concat(axis=0, values=[node_c, c])
                idx_var = tf.add(idx_var, 1)
                logging.warning('get new node_h and new node_c done')
                return node_h, node_c, idx_var
            ...
            node_h, node_c, idx_var = tf.while_loop(loop_cond, _recurceleaf, loop_vars,
                                                    shape_invariants=[tf.TensorShape([None, self.hidden_dim]),
                                                                      ...
                                                                      idx_var.get_shape()])
            logging.warning('return new node_h, finished')
